...
function renderQuizStageUI(stage) {
  const isVol = stage === 'vol';
  quizTabVol.classList.toggle('active', isVol);
  quizTabArea.classList.toggle('active', !isVol);
  quizTabVol.classList.toggle('done', !isVol);
  quizTabArea.classList.toggle('done', false);
...
quizNext.addEventListener('click', () => {
  if (current && current.stage === 'vol') {
    // Lanjut ke tahap luas permukaan untuk bentuk & dimensi yang sama
    startQuestion(current.key, current.r, current.t, 'area');
  } else {
    newQuestion();
  }
});
...
function checkAnswer() {
  ...
  quizNext.textContent = stage === 'vol' ? 'Lanjut: Luas permukaan →' : 'Pertanyaan baru';
