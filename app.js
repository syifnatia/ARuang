...
/* Referensi entitas untuk fitur "Jaring-jaring Tabung" (hanya berlaku untuk tabung) */
const NET = {
  group: $('cyl-net'),
  rect: $('net-rect'),
  circleTop: $('net-circle-top'),
  circleBottom: $('net-circle-bottom'),
  wRuler: $('net-wruler'), wLabel: $('net-wlabel'),
  hRuler: $('net-hruler'), hLabel: $('net-hlabel'),
  rLabel: $('net-rlabel'),
  seam: $('cyl-seam'),
};

/* Menghitung & menata ulang layout jaring-jaring (2 lingkaran + persegi panjang selimut)
   berdasarkan jari-jari (r) dan tinggi (t) tabung saat ini. Dipanggil terus-menerus
   di dalam updateGeometry() agar tetap sinkron walau elemen sedang tersembunyi. */
function updateNetGeometry(r, h) {
  const mr = r * MODEL_SCALE, mh = h * MODEL_SCALE;
  const keliling = 2 * PI * mr; // lebar persegi panjang selimut = keliling alas

  NET.rect.setAttribute('width', keliling);
  NET.rect.setAttribute('height', mh);
  NET.rect.setAttribute('position', `0 ${mh / 2} 0`);

  NET.circleTop.setAttribute('radius', mr);
  NET.circleTop.setAttribute('position', `0 ${mh + mr} 0`);
  NET.circleBottom.setAttribute('radius', mr);
  NET.circleBottom.setAttribute('position', `0 ${-mr} 0`);

  NET.wRuler.setAttribute('width', keliling);
  NET.wRuler.setAttribute('position', '0 -0.02 0');
  NET.wLabel.setAttribute('position', '0 -0.05 0');

  const hx = -(keliling / 2 + 0.03);
  NET.hRuler.setAttribute('height', mh);
  NET.hRuler.setAttribute('position', `${hx} ${mh / 2} 0`);
  NET.hLabel.setAttribute('position', `${hx - 0.03} ${mh / 2} 0`);

  NET.rLabel.setAttribute('position', `${mr + 0.03} ${mh + mr} 0`);

  NET.seam.setAttribute('height', mh);
  NET.seam.setAttribute('position', `${mr} ${mh / 2} 0`);
...
function updateGeometry(r, h) {
  ...
  if (state.net) {
    // Saat jaring-jaring tampil, panel rumus melayang di atas seluruh layout net
    SOLIDS.cyl.label.setAttribute('position', `0 ${mh + 2 * mr + 0.16} 0`);
  }
...
btnWire.addEventListener('click', () => {
  ...
  NET.rect.setAttribute('material', `wireframe: ${wfVal}`);
  NET.circleTop.setAttribute('material', `wireframe: ${wfVal}`);
  NET.circleBottom.setAttribute('material', `wireframe: ${wfVal}`);
});
...
/* -----------------------------------------------------------------
   FITUR: JARING-JARING TABUNG
   Membuka tabung menjadi 2 lingkaran (alas & tutup) + 1 persegi
   panjang (selimut, lebar = keliling alas). Hanya berlaku utk tabung.
   ----------------------------------------------------------------- */
const btnNet = $('btn-net');
let netTimers = [];

function runNetAnimation() {
  netTimers.forEach(clearTimeout); netTimers = [];
  const s = SOLIDS.cyl;
  const { r, h } = readSliders();

  // Reset animasi sebelumnya (jika fitur dijalankan berulang kali)
  NET.rect.removeAttribute('animation__unroll');
  NET.circleTop.removeAttribute('animation__pop');
  NET.circleBottom.removeAttribute('animation__pop');
  NET.rect.object3D.scale.set(1, 1, 1);
  NET.circleTop.setAttribute('scale', '1 1 1');
  NET.circleBottom.setAttribute('scale', '1 1 1');
  updateNetGeometry(r, h);

  const DUR = 2200;
  const at = (t, fn) => netTimers.push(setTimeout(fn, t));

  // TAHAP 1: tampilkan tabung utuh + garis potong tegak (rencana pembelahan)
  ...
  NET.group.setAttribute('visible', false);
  NET.seam.setAttribute('visible', true);
  NET.seam.setAttribute('animation__pulse', 'property: material.emissiveIntensity; from: 0.3; to: 1.4; dir: alternate; loop: true; dur: 450; easing: easeInOutSine');

  setLabel(s.label, [
    { text: 'JARING-JARING TABUNG', size: 46, color: '#eab308', weight: '800' },
    { text: 'Selimut digunting tegak, alas & tutup dilepas', size: 36, color: '#F8FAFC' },
  ]);

  // TAHAP 2: buka jadi jaring-jaring datar (selimut "digulung buka", tutup & alas muncul)
  at(DUR, () => {
    s.shape.setAttribute('visible', false);
    NET.seam.removeAttribute('animation__pulse');
    NET.seam.setAttribute('visible', false);
    NET.group.setAttribute('visible', true);

    const { mh, keliling } = updateNetGeometry(r, h);
    NET.rect.object3D.scale.x = 0.02;
    NET.rect.setAttribute('animation__unroll', `property: object3D.scale.x; from: 0.02; to: 1; dur: ${DUR}; easing: easeOutQuad`);
    NET.circleTop.setAttribute('scale', '0.02 0.02 0.02');
    NET.circleBottom.setAttribute('scale', '0.02 0.02 0.02');
    NET.circleTop.setAttribute('animation__pop', `property: scale; from: 0.02 0.02 0.02; to: 1 1 1; dur: ${DUR}; easing: easeOutBack`);
    NET.circleBottom.setAttribute('animation__pop', `property: scale; from: 0.02 0.02 0.02; to: 1 1 1; dur: ${DUR}; easing: easeOutBack`);

    setLabel(s.label, [
      { text: '2 Lingkaran = alas & tutup', size: 40, color: '#A78BFA', weight: '800' },
      { text: '1 Persegi panjang = selimut', size: 40, color: '#38BDF8', weight: '800' },
    ]);
  });

  // TAHAP 3: rumus luas permukaan dari komponen jaring-jaring
  at(DUR * 2, () => {
    setLabel(s.label, [
      { text: 'Lp = 2×(π·r²) + (2π·r × t)', size: 36, color: '#F8FAFC', weight: '700' },
      { text: `Lp = 2πr(r + t) = ${fmt(cylinderArea(r, h))} cm²`, size: 38, color: '#22c55e', weight: '800' },
    ]);
  });
}
...
function enterNet() {
  ...
  if (state.current !== 'cyl') {
    spawnShape('cyl');
    showStatusToast('Jaring-jaring hanya berlaku untuk tabung');
  }
  cancelBreakdown();
  setExpanded(true);
  state.net = true;
...
function exitNet() {
  ...
  NET.seam.removeAttribute('animation__pulse');
  NET.seam.setAttribute('visible', false);
  SOLIDS.cyl.shape.setAttribute('visible', true);
  SOLIDS.cyl.rRuler.setAttribute('visible', true);
  SOLIDS.cyl.rLabel.setAttribute('visible', true);
  SOLIDS.cyl.hRuler.setAttribute('visible', true);
  SOLIDS.cyl.hLabel.setAttribute('visible', true);
  if (visible.has('cyl') && !state.quiz) setDefaultLabel('cyl');
  refresh();
}
...
async function startAR() {
  ...
  try {
    // PENTING: enterAR()/enterVR() mengembalikan Promise. Sebelumnya promise ini
    // tidak ditunggu (await), jadi UI langsung dianggap "masuk AR" walau sesinya
    // gagal diam-diam di belakang (mis. ARCore belum terpasang/diperbarui di
    // Android) — akibatnya overlay & tombol muncul di atas layar putih kosong
    // tanpa feed kamera sama sekali, persis seperti kondisi macet di "Cari
    // permukaan datar…".
    if (sceneEl.enterAR) await sceneEl.enterAR();
    else await sceneEl.enterVR(true);

    // Baru ubah UI SETELAH sesi AR benar-benar aktif
    introBox.classList.add('hidden');
    statusEl.textContent = 'Cari permukaan datar…';
    showPlaceHint();
  } catch (e) {
    console.warn('Gagal memulai sesi AR:', e);
    enterFallback('Tidak bisa memulai sesi AR (ARCore/Google Play Services for AR mungkin belum terpasang atau perlu diperbarui, atau kamera sedang dipakai aplikasi lain). Beralih ke simulator 3D.');
  }
}
